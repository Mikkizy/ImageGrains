# Logbook
Meeting (23 May 2025)

    Present: Miracle Ukaka (MU, student), Rebecca Bell (RB, main supervisor), Nahin Rezwan (NR, second supervisor)

    Key points discussed:

        RB explained the scope of the project and problems the project intends to tackle.

        RB provided clarification on the issues associated with grain-size segmentation.

        NR mentioned the availability of some data obtained from field work.

        RB and NR mentioned the need of fixing a stable meeting time.

    Feedback received:

        RB advised that we read two papers that reviews existing work on the project scope.

    Work plan before next meeting:

        Review the papers and talk about what I understand from them.


Meeting (9 June 2025)

    Present: Miracle Ukaka (MU, student), Rebecca Bell (RB, main supervisor), Nahin Rezwan (NR, second supervisor)

    Key points discussed:

        Discussed our understanding of the papers provided.

        Scheduled a fixed meeting time

        Discussed the project plan and the path to take.

    Feedback received:

        The team accepted the mobile development path, and saw a need for its usecase in field and laboratory work.

    Work plan before next meeting:

        Submit initial draft of project plan for review.

        Start reviewing existing literatures.

Meeting (23 June 2025)

    Present: Miracle Ukaka (MU, student), Rebecca Bell (RB, main supervisor), Nahin Rezwan (NR, second supervisor)

    Key points discussed:

        Steps needed for converting models

        The need for the right scaling strategy

    Feedback received:

        The team felt the project plan was resourceful and encouraged more research into the right conversion implementations.

    Work plan before next meeting:

        Convert models into tensorflow lite and check accuracy trade-offs.

Meeting (3 July 2025)

    Present: Miracle Ukaka (MU, student), Rebecca Bell (RB, main supervisor), Nahin Rezwan (NR, second supervisor)

    Key points discussed:

        Model Conversion Results

        Accuracy Trade-offs

        Image Scaling and Downsizing

    Feedback received:

        The team was okay with the converted model results and suggested that I track the changes.

    Work plan before next meeting:

        Implement base mobile app with integrated models.

Meeting (14 July 2025)

    Present: Miracle Ukaka (MU, student), Rebecca Bell (RB, main supervisor), Nahin Rezwan (NR, second supervisor)

    Key points discussed:

        Android Application Design and Tests

        Features Integration

    Feedback received:

        The team was okay with the tests and suggested that I added D16 and D84 results. 

    Work plan before next meeting:

        Add feature for storage and localization.
        
        Run tests on Nahin's images.

Meeting (21 July 2025)

    Present: Miracle Ukaka (MU, student), Rebecca Bell (RB, main supervisor), Nahin Rezwan (NR, second supervisor)

    Key points discussed:

        New Features Added

        Field Trip for Testing

    Feedback received:

        The team was okay with the new save session feature.

    Work plan before next meeting:

        Go to field trip to test camera functionality.
        
        Review some more papers.

